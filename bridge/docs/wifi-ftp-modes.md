...
- Pi SSID: `InstBrdg-XXXXXXXX`, where `XXXXXXXX` is the bridge's device suffix
...
`InstBrdg-XXXXXXXX` by default, and uploads to `192.168.8.1`. No existing Wi-Fi password is needed for
the bridge workflow.
...
```bash
printf '%s\n' 'InstBrdg-1234ABCD' | sudo tee /etc/InstantLinkBridge/hotspot.ssid
