...
WIDTH = 240
HEIGHT = 240
# Brand ink base (palette "Ink" #1A1613) — enough visual feedback that the
# bridge is booting without needing a font/text renderer. The normal boot path
# instead `dd`s the pre-rendered logo splash (assets/boot-splash.rgb565,
# regenerated by brand/render-assets.sh); this solid fill is the Python
# fallback, so keep it on the same brand ground.
SPLASH_COLOR_RGB = (26, 22, 19)
