...
WIDTH = 240
HEIGHT = 240
# Brand teal base (palette "Teal deep" #0B3A44) — enough visual feedback that
# the bridge is booting without needing a font/text renderer. The normal boot
# path instead `dd`s the pre-rendered logo splash (assets/boot-splash.rgb565,
# regenerated by brand/render-assets.sh); this solid fill is the Python
# fallback, so keep it on the same brand base.
SPLASH_COLOR_RGB = (11, 58, 68)
