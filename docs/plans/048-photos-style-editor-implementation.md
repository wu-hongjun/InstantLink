...
| #17 Final polish + fidelity pass + version bumps | ✅ landed | `adbcfc7` — option-drag extended range + VoiceOver on AdjustmentSlider, LightPipeline contrast multiplier tune, 100 ms histogram debounce confirmed, 7-locale (de/es/fr/it/pt-BR/ar/he) backfill for editor tab bar + section titles + Light slider labels, stale editor_coming_in_pr_2/3 keys dropped, PR #14 audit L1–L4 nits cleared (annotate_overlay_lock/unlock/show/hide help text + OverlayItem.displayTitle DRY + EditorPreview film-frame Q4 comment + annotate_select_image_overlay key), crate versions verified at 0.1.45 |
