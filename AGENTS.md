...
Build the release app bundle with `bash scripts/build-app.sh 0.1.2`. The macOS app baseline is 15.0 (`arm64-apple-macosx15.0` + `LSMinimumSystemVersion 15.0`). Install the CLI locally with `cargo install --path crates/instantlink-cli`. Preview docs with `mkdocs serve` after `pip install mkdocs-material`.
